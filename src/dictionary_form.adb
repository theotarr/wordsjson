...
with LIST_PACKAGE; use LIST_PACKAGE;
  ...
  function DICTIONARY_FORM(DE : DICTIONARY_ENTRY) return STRING is
      ...
      begin
        FORM := HEAD(TRIM(FORM) & TRIM(FACTOR), 250);
      end ADD_UP;

      procedure ADD_TO(FACTOR : STRING) is
      begin
        FORM := HEAD(TRIM(FORM) & FACTOR, 250);
      end ADD_TO;
...
--TEXT_IO.PUT_LINE(OX(1) & "+" & OX(2) & "+" & OX(3) & "+" & OX(4));                
        ...
        ADD_TO(PART_OF_SPEECH_TO_XML(DE.PART.POFS));

      if DE.PART.POFS = N  then
        if DE.PART.N.DECL.WHICH in 1..5 and
           DE.PART.N.DECL.VAR  in 1..5 then
             ADD_TO(DECL_TO_XML(DE.PART.N.DECL, FALSE));
         end if;         
        
        ADD_TO(GENDER_TO_XML(DE.PART.N.GENDER));
      end if;
      
      if (DE.PART.POFS = V)  then 
        if DE.PART.V.CON.WHICH in 1..3 then
           if DE.PART.V.CON.VAR = 1 then
             ADD_TO(CONJ_TO_XML(DE.PART.V.CON, FALSE));
           elsif  DE.PART.V.CON = (3, 4)  then
             FUDGE_CON:
             declare
               TEMP_CON : DECN_RECORD := DE.PART.V.CON;
             begin
               TEMP_CON.WHICH := 4;
               ADD_TO(CONJ_TO_XML(TEMP_CON, FALSE));
             end FUDGE_CON;
           end if;
         end if;         
        
         if  (DE.PART.V.KIND in GEN..PERFDEF)  then
           ADD_TO(VERB_KIND_TO_XML(DE.PART.V.KIND));
         end if;
