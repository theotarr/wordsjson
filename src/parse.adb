...
procedure PARSE(COMMAND_LINE : STRING := "") is
  ...
  begin
    ...
    -- start wrapping tag
    if LANGUAGE = LATIN_TO_ENGLISH
    then
      if  WORDS_MODE(WRITE_OUTPUT_TO_FILE)
      then
        TEXT_IO.PUT_LINE(OUTPUT, "<words>");
      else
        TEXT_IO.PUT_LINE(CURRENT_OUTPUT, "<words>");
      end if;
    end if;
...
--                or (CHARACTER'POS(LINE(I)) < 32)  or (CHARACTER'POS(LINE(I)) > 127) );
      ...
      for I in J..K-1               --  Kludge for QVAE
      loop
      ...
      elsif LANGUAGE = LATIN_TO_ENGLISH
      then
        ...
        procedure PASS(INPUT_WORD : STRING);
          ...
          begin
            ...
            for I in 1..ENCLITIC_LIMIT      --  If have parse, only do que of que, ne, ve, (est)
            loop
              REMOVE_A_TACKON:
              declare
              ...
              --TEXT_IO.PUT_LINE("In ENCLITIC     LESS/TACKON  = " & LESS & "/" & TACKONS(I).TACK);
                ...
                then
                  ...
                  SYPA_LAST := 0;
                  if WORDS_MDEV(DO_SYNCOPE) and not NO_SYNCOPE
                  then
                  ...
                  if PA_LAST > ENTERING_PA_LAST      --  have a possible word
                  then
                    PA_LAST := PA_LAST + 1;
            ...
            loop
              ...
              begin
                ...
                then
                  ...
                  if TRPA_LAST > ENTERING_TRPA_LAST        --  have a possible word
                  then
...
--  For this we have to look ahead
          ...
          if PA_LAST > 0       --  But PA may be killed by ALLOW in LIST_STEMS
          then
...
(("zzz      ", "zzz      ", "zzz      "), ("zzz      ", "zzz      ", "zzz      "))
              ...
              begin
                --  Look ahead for sum
                LOOK_AHEAD;
                if IS_SUM(NEXT_WORD)                   --  On NEXT_WORD = sum, esse, iri
                then
                  for I in 1..PA_LAST      --  Check for PPL
                  loop
                  ...
                  if K = NK        --  There was a PPL hit
                  then
                    CLEAR_PAS_NOM_PPL:
                    declare
                      J : INTEGER := PA_LAST;
                    begin
                      while J >= 1          --  Sweep backwards to kill empty suffixes
                      loop
                        if ((PA(J).IR.QUAL.POFS = PREFIX) and then (PPL_ON))
                        then
                          null;
                        elsif ((PA(J).IR.QUAL.POFS = SUFFIX) and then (PPL_ON))
                        then
                          null;
                        elsif ((PA(J).IR.QUAL.POFS = TACKON) and then (PPL_ON))
                        then
                          null;
                        elsif PA(J).IR.QUAL.POFS = VPAR and then
                              PA(J).IR.QUAL.VPAR.CS = NOM  and then
                              PA(J).IR.QUAL.VPAR.NUMBER = SUM_INFO.NUMBER
                        then
                          if PA(J).IR.QUAL.VPAR.TENSE_VOICE_MOOD = (PERF, PASSIVE, PPL)
                          then
                            PPL_ON := TRUE;
                          ...
                          elsif PA(J).IR.QUAL.VPAR.TENSE_VOICE_MOOD = (FUT, ACTIVE,  PPL)
                          then
                            PPL_ON := TRUE;
                            ...
                            PPL_INFO := (PA(J).IR.QUAL.VPAR.CON,   --  In this case, there is 1
                                         ...
                                         PA(J).IR.QUAL.VPAR.TENSE_VOICE_MOOD);
                            PPP_MEANING :=
                              HEAD("FUT ACTIVE PPL + verb TO_BE => ACTIVE Periphrastic - about to, going to",
                                   MAX_MEANING_SIZE);
  
                          elsif PA(J).IR.QUAL.VPAR.TENSE_VOICE_MOOD = (FUT, PASSIVE, PPL)
                          then
                            PPL_ON := TRUE;
                            COMPOUND_TENSE := SUM_INFO.TENSE_VOICE_MOOD.TENSE;
                            COMPOUND_TVM := (COMPOUND_TENSE, PASSIVE, SUM_INFO.TENSE_VOICE_MOOD.MOOD);
                            ...
                            PPL_INFO := (PA(J).IR.QUAL.VPAR.CON,   --  In this case, there is 1
                                         ...
                                         PA(J).IR.QUAL.VPAR.TENSE_VOICE_MOOD);
                            PPP_MEANING :=
                            HEAD("FUT PASSIVE PPL + verb TO_BE => PASSIVE Periphrastic - should/ought/had to",
                                 MAX_MEANING_SIZE);
                          end if;
                        else
                ...
                elsif IS_ESSE(NEXT_WORD) or IS_FUISSE(NEXT_WORD)  --  On NEXT_WORD
                then
                  for I in 1..PA_LAST      --  Check for PPL
                  loop
                    if PA(I).IR.QUAL.POFS = VPAR and then
                        ...
                        then
                          ...
                          then
                            ...
                            PPL_INFO := (PA(J).IR.QUAL.VPAR.CON,   --  In this case, there is 1
                                         ...
                                         PA(J).IR.QUAL.VPAR.GENDER,--  all have same PPL_INFO
                                         PA(J).IR.QUAL.VPAR.TENSE_VOICE_MOOD);
                            if IS_ESSE(NEXT_WORD)
                            then
                              COMPOUND_TVM := (FUT, ACTIVE, INF);
                              PPP_MEANING :=
                                HEAD("FUT ACTIVE PPL + esse => PRES Periphastic/FUT ACTIVE INF - be about/going to",
                                     MAX_MEANING_SIZE);
                                -- also peri COMPOUND_TVM := (PRES, ACTIVE, INF);
                            else   --  fuisse
                              COMPOUND_TVM := (PERF, ACTIVE, INF);
                              PPP_MEANING :=
                                HEAD("FUT ACT PPL+fuisse => PERF ACT INF Periphrastic - to have been about/going to",
                                     MAX_MEANING_SIZE);
                            end if;
                          ...
                          then
                            ...
                            then
                              ...
                              PPP_MEANING :=
                                HEAD("FUT PASSIVE PPL + esse => PRES PASSIVE INF",
                                     MAX_MEANING_SIZE);
                              -- also peri COMPOUND_TVM := (PRES, ACTIVE, INF);
                            else   --  fuisse
                              COMPOUND_TVM := (PERF, PASSIVE, INF);
                              PPP_MEANING :=
                                HEAD("FUT PASSIVE PPL + fuisse => PERF PASSIVE INF Periphrastic - about to, going to",
                                     MAX_MEANING_SIZE);
                            end if;
                          end if;
                        else
                ...
                elsif IS_IRI(NEXT_WORD)                --  On NEXT_WORD = sum, esse, iri
                then
                  --  Look ahead for sum
                  for J in 1..PA_LAST      --  Check for SUPINE
                  loop
                  ...
                  if K = NK        --  There was a SUPINE hit
                  then
                    CLEAR_PAS_SUPINE:
                    declare
                      J : INTEGER := PA_LAST;
                          ...
                          PA(PA_LAST) :=
                              (HEAD("SUPINE + iri", MAX_STEM_SIZE),
                                    ((V,
                                        ...
                                        PPP, NULL_MNPC);
                          PPP_MEANING :=
                            HEAD("SUPINE + iri => FUT PASSIVE INF - to be about/going/ready to be ~",
                                 MAX_MEANING_SIZE);
...
--
  ...
  then
    ...
    if CONFIGURATION = ONLY_MEANINGS
    then
      PREFACE.PUT_LINE(
    ...
    loop
      ...
      begin                    --  Block to manipulate file of lines
        ...
        then
          ...
          if (NAME(CURRENT_INPUT) = NAME(STANDARD_INPUT))     --  INPUT is keyboard
          then
            PREFACE.PUT("Blank exits =>");
            GET_LINE(LINE, L);             -- Second try
            if (L = 0) or else (TRIM(LINE(1..L)) = "")    -- Two in a row
            then
              exit;
            end if;
          else                 --  INPUT is file
        ...
        if (TRIM(LINE(1..L)) /= "")    -- Not a blank line so L(1) (in file input)
        then
          if LINE(1) = START_FILE_CHARACTER      --  To begin file of words
          then
            if (NAME(CURRENT_INPUT) /= NAME(STANDARD_INPUT))
            then
          ...
          then
            ...
            if (NAME(CURRENT_INPUT) /= NAME(STANDARD_INPUT))
            then
              PREFACE.PUT_LINE("<phrase>" & LINE(1..L) & "</phrase>");
            end if;
            if WORDS_MODE(WRITE_OUTPUT_TO_FILE)
            then
              if not CONFIG.SUPPRESS_PREFACE
              then
                TEXT_IO.PUT_LINE(OUTPUT, "<phrase>" & LINE(1..L) & "</phrase>");
              end if;
            end if;
  ...
  begin
    ...
    if STEM_IO.IS_OPEN(STEM_FILE(LOCAL))
    then
  ...
  exception
    when others =>
      null;    --  not there, so don't have to DELETE
  end;
  begin
    if
