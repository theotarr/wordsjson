...
--  Need to carry LOC through consistently on LOAD_D and LOAD_D_FILE
  ...
  begin
    ...
    PUT(MDEV_FILE, "DATA_FILE_DIRECTORY               " &
                    DATA_FILE_DIRECTORY); NEW_LINE(MDEV_FILE);
    CLOSE(MDEV_FILE);
  end PUT_MDEVS;
  ...
  begin
    ...
    GET_LINE(MDEV_FILE, LINE, LAST);
    if LINE(1..19) = "DATA_FILE_DIRECTORY" then
      DATA_FILE_DIRECTORY(1..(LAST-35+1)) := LINE(35..LAST);
    else
      raise BAD_MDEV_FILE;
    end if;
...
begin
  ...
  --  Put that the MDEV file is corrupted and the options are:
    ...
    when BAD_MDEV_FILE  =>
      PREFACE.PUT_LINE("MDEV_FILE exists, but empty or corrupted - Default MDEVs used");
